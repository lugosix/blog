...
引用自 https://www.jianshu.com/p/cffffa914381

## 2.4 感悟&思考
### 2.4.1 为什么适合读多写少
- 数据结构特点，存在一个不用加锁的只读 map

### 2.4.2 为什么需要 amended 
- 复用现有 map 数据结构，不用额外做改造

### 2.4.3 为什么需要 expunged
为了保持多读少写情况下的高性能

如果不这么做

- 直接删：readOnly 要加锁
- 置为 nil，不复制到 dirty：如果有相同 key 写入，map 切换时，漏数据
- 置为 nil，复制到 dirty：key 永远不会被释放
### 2.4.4 存在的性能风险
- readOnly 到 dirty 的拷贝
- 写多了之后性能退化严重
- key 内存释放不及时
- 按照 miss 次数做替换，p99 可能不稳定
### 2.4.5 没有多读少写还想线程安全怎么办
- lock-free map
- rwmutex 等
- map 分片，每片加锁